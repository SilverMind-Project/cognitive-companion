...
- **R2 / M33:** The four CTS filters (`room`, `room_transition`, `person_presence`, `scene_trend`) were migrated to `PersonLocationService` as the single source of truth (`presence_segments` / `location_observations`). The legacy tables (`person_location_state`, `person_location_history`, `person_sightings`), step-level write flags (`record_presence`, `record_sightings`), and `LocationWriter` were fully decommissioned and dropped in **M33**. No code may reference those deleted tables or writer.
- **Person location has one read API: `PersonLocationService`** (`where_is`, `where_is_everyone`, `presence_history`, `room_segments`, `observations`, `recent_observations`, `latest_observation`, `latest_floor_point`, envelopes). Never query `location_observations` / `presence_segments` directly from services or steps, and never reintroduce a second location store or a second query path for the same data. As of **M38**, a same-room repeat observation refreshes the open segment's `last_observed_at`/`confidence`/`quality` (the state machine's refresh branch, not a no-op), so `where_is()`'s segment is no longer frozen at entry time either; `latest_observation()` remains the observation-level freshness/staleness signal regardless of segment state, and still answers after a segment has closed or quiet-timed-out. Segments age out per source (`PersonLocationConfig.quiet_gap_s`): a dense source (`world_tracker`) ages out fast, a sparse rule-driven source (`face_sighting`) stays credible far longer, and `manual` never ages. As of **M39**, presence provider `LocationServiceProvider` (`location_service`) uses a per-source freshness matrix (`ttl_seconds_by_source`) matching the SSOT quiet gaps so reCamera face sightings and sensors retain presence credibility when camera tracking is offline. `PresenceService` is un-gated from CTS and runs unconditionally in lifespan. `OccupancyReadModel` accepts `face_sighting` presence for identified persons and named guests under stable hypothesis key `f"face_{person_id}"`.
- **`PersonLocationService` is constructed unconditionally** (`perception.wire_perception`, M38): it depends only on `get_session`, not on `cts.enabled`. reCamera identification (`services/person_tracking.py` via `RecameraLocationIngest`) and HA presence correlation (`PersonTrackingService._correlate_presence_sensor`) write to it directly, independent of CTS. **Every location source writes through `PersonLocationService.ingest_*`.** Source vocabulary is `world_tracker`, `face_sighting`, `sensor`, `manual` (SourceTags name evidence pathways, not hardware). A new source gets a `SourceTag`, an arbitration priority (`services/person_location/source_arbitration.py`), and a quiet gap (`PersonLocationConfig`); it never gets a second store, a second writer service, or a step-level write flag. Observation rows are unarbitrated history (`ingest_observation` always inserts one); segments are the arbitrated present (`source_arbitration.arbitrate` gates only a room-*changing* observation against the open segment's `last_source`/`last_observed_at`, with an out-of-order guard that never lets a lagged event move evidence backward).
- **One `image_source` vocabulary via shared resolver (M34).** Perception steps (`image_crop`, `person_identification`, `scene_analysis`, `llm_call`) declare a single consolidated `image_source` vocabulary resolved by `ImageSourceResolver`. The deprecated `cts_window` and `cts_frames_path` options are removed.
- **Signal vocabularies derive from `cts_contracts` (M35/M37).** `ALL_SIGNAL_KINDS` in `signal_config.py` derives dynamically from `cts_contracts.DementiaSignalKind` plus `CC_LOCAL_SIGNAL_KINDS`. Preset profile memberships (`_PRESENCE_KINDS` etc.) carry guard tests asserting every member is in `ALL_SIGNAL_KINDS`.
- **`observations()` is per-frame, not per-event.** `world_tracker` can call `ingest_observation()` several times a second (tracking-orchestrator publishes at up to 3Hz per camera); the legacy `PersonSighting` table it replaced for the activity timeline's "sighting" source was written once per identification run (sparse). Event-grade consumers use `bucketed_observations()` (one representative row per room per time bucket, deduped by a SQL window function **before** LIMIT applies, so wide windows keep their early history), never raw `observations()` per-row: see `ActivityTimelineService._get_sighting_events`. A client-side throttle over a LIMITed raw query is the known-wrong shape (it silently drops old history once raw rows exceed the limit).
- **Seed location state through the service's ingest methods, never raw repo inserts.** Tests (and any backfill/tooling) create presence state via `ingest_observation` / `ingest_room_transition` so the segment state machine enforces the one-open-segment-per-person invariant; two raw-inserted "open" segments make `get_open`/`where_is` return an arbitrary one (a real bug hit and fixed in the M32 test rewrite). Closed historical segments written by a sanctioned backfill path are the one exception, and they go through a dedicated service method, not repo calls from outside the package.
