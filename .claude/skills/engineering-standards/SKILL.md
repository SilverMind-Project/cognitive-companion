...
- **`observations()` is per-frame, not per-event.** `world_tracker` can call `ingest_observation()` once per frame; the legacy `PersonSighting` table it replaced for the activity timeline's "sighting" source was written once per identification run (sparse). Any consumer that turns raw observations into one UI/timeline event per row must throttle client-side (see `ActivityTimelineService._get_sighting_events`'s `_SIGHTING_MIN_INTERVAL_S` dedup) or a dense observation stream will drown out every other event source once merged and limit-capped. Don't assume `observations()` rows are already event-grade; they're the raw feed.
