...
- **Person location has one read API: `PersonLocationService`** (`where_is`, `where_is_everyone`, `presence_history`, `room_segments`, `observations`, `latest_observation`, `latest_floor_point`, envelopes). Never query `location_observations` / `presence_segments` directly from services or steps, and never reintroduce a second location store or a second query path for the same data. `latest_observation()` (not `where_is()`) is the freshness/staleness signal for presence providers: `where_is()`'s open segment is frozen at entry time for repeat same-room observations (the state machine no-ops them), so it under-reports how recently a person was actually seen.
