...
### 16.1 Correctness expectations for any change

Before opening a PR, every code change must:

1. **Pass `make check`** (lint + strict mypy on `backend.core` + `backend.tests.core`). For service or schema changes, also pass `make check-all`.
2. **Have unit tests for new code.** Public classes, public methods, and new endpoints get mirrored tests under `backend/tests/<mirror_path>/`. New step handlers, channels, and filters each get at least: success path, missing-service path, and one config-edge-case test.
3. **Use strongly-typed objects at boundaries.** `@dataclass(frozen=True)` for results returned to callers, Pydantic for HTTP wire models. No `dict[str, Any]` leaking out of an integration client.
4. **Round-trip datetimes.** External datetimes pass through `backend.core.time.normalize_utc_datetime()` before being stored. Outbound serializes with `.astimezone(UTC).isoformat()`.
5. **Honor graceful degradation.** Every integration client returns `None`, `[]`, or a typed zero value when the upstream is disabled or unreachable. No exceptions bubble.
6. **Add an `auth.yaml` entry** for any new HTTP route, or extend an existing pattern where appropriate.
7. **Run the typecheck against any module you edited.** `make typecheck-core` is mandatory; `make typecheck` on the changed module is a strong recommendation.
8. **Verify the frontend.** `cd frontend && npm run build` for any frontend change. If new step types or filter types are added, also run `npm run test`.
...
- Do not skip permission checks. Every endpoint needs an `auth.yaml` entry.
...
- Do not call `toLocaleString()`/`toLocaleDateString()`/`toLocaleTimeString()` directly in the frontend. Use `services/timezone.js`.
...
- Do not import `ObjectTrendClient` or `backend.integrations.object_trend_client`; that module was deleted. Use `SemanticMemoryClient` from `backend.integrations.semantic_memory_client`.
...
- Do not add code paths that depend on the deprecated `services.person_tracking` or `services.activity_session_service`. Use `services.activity` (`ActivityService`).
