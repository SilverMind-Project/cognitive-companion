...
Current built-in step types: `activity_detection`, `activity_session_start`, `activity_session_end`, `condition`, `daily_report`, `gate_verdict`, `guided_task_start`, `ha_action`, `home_state`, `image_crop`, `info_card`, `interactive_prompt`, `llm_call`, `media_presign`, `media_window_poll`, `notification`, `novelty_gate`, `object_trend_analysis`, `person_identification`, `presence_query`, `quiz_start`, `region_presence`, `scene_analysis`, `semantic_memory_query`, `semantic_memory_write`, `signal_emit`, `verification`, `wait`. `media_window_poll` is the single camera polling step for both CTS and reCamera sources (select via its `source` config: `auto`, `cts`, or `recamera`). The former `cts_window_poll` and `recamera_media_poll` step types have been removed. `region_presence` tests person bboxes from `scene_detections` against configured normalized image-space regions (rects or polygons) without a model call; see `backend/steps/_region_geometry.py` for the coordinate-space rules. `signal_emit` writes a CC-local signal (e.g. `tea_intent_suspected`) via `SignalsService.emit()`, validated against `signal_config.CC_LOCAL_SIGNAL_KINDS`; it is the label store for shadow-mode detectors (DL10). `media_presign` resolves MinIO object names referenced in trigger or pipeline context (e.g. a `dementia_signal`'s evidence keyframes) into presigned URLs for downstream steps. `novelty_gate` compares a CLIP embedding (e.g. `scene_analysis`'s `scene_embedding`) against the last one cached for its scope and emits `novel`/`distance`/`reason`, fail-open on a missing embedding; authoring pattern is `scene_analysis -> novelty_gate -> condition(novel == true) -> llm_call` (DL5/DL-M09).
