...
Start with [docs/systems-architecture.md](docs/systems-architecture.md): the CC-focused architecture reference (sensors and integrations, rules engine and trigger types, event aggregation, plugin systems, the PWA companion view, the Gemini Live realtime voice path, and a current bugs-and-gaps checklist). The CTS-side companion doc is [`../continuous-tracking/docs/systems-architecture.md`](../continuous-tracking/docs/systems-architecture.md).

1. [AGENTS.md](AGENTS.md): canonical reference (architecture, plugin contracts, the registered step types, channels, filters, CTS gateway, per-person signal config, testing conventions, naming conventions, common tasks). Note: the registry now has 23 step types (`cts_window_poll`, `recamera_media_poll`, and `image_crop` are not yet in the AGENTS.md list); see docs/systems-architecture.md.
...
- **CTS identity model: `ph_id` is the single cross-camera identifier.** The CTS orchestrator runs a Bayesian identity resolver on each PersonHypothesis (PH). When the identity changes, an `IdentityRevision` proto arrives on `tracking.revisions`; `IdentityRevisionSubscriber` soft-deletes the superseded `PersonLocationHistory` row and inserts a corrected replacement. CC never computes identity: read it from `TrackingEvent.identity_id` and write corrections only through the `cts_identity` router. Cross-camera identity propagates via: (1) pre-association dedup for calibrated overlapping cameras, (2) the shared ReID gallery, (3) cross-GT face propagation, and (4) PH continuation candidates; CC receives the result as a single `ph_id` regardless of which path was used.
