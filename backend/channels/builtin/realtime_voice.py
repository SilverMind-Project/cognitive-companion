"""Realtime voice notification channel.
...
Pair this channel with ``websocket`` or ``telegram`` to ensure delivery when
the companion UI is not open.
"""
...
class RealtimeVoiceChannel(NotificationChannel):

    @classmethod
    def metadata(cls) -> ChannelMetadata:
        return ChannelMetadata(
            channel_name="realtime_voice",
            display_name="Realtime Voice",
            description=(
    ...
    async def send(
        ...
        try:
            ...
            logger.error("realtime_voice_dispatch_failed", error=str(e))
            return False
