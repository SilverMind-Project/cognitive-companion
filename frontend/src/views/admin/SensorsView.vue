    ...
    <div class="d-flex align-center mb-6">
      <div>
        ...
        <div class="text-body-2 text-medium-emphasis mt-1">
          Cameras, motion sensors, and other inputs the system listens to.
        </div>
      </div>
      <v-spacer />
          ...
          <div class="pa-6 text-center">
            <v-card flat>
              ...
              <v-card-text class="text-grey"
                >Add sensors or import from Home Assistant to begin monitoring.</v-card-text
              >
            </v-card>
          </div>
        </template>
    ...
    <v-dialog v-model="dialog" max-width="500" persistent>
      <v-card>
        ...
        <v-card-text>
          ...
          />
          <v-text-field
            v-model="form.ha_entity_id"
            label="HA Entity ID (optional)"
            variant="outlined"
            class="mb-2"
          />
...
async function loadData() {
  loading.value = true;
  try {
    const [rawSensors, rooms] = await Promise.all([api.getSensors(), api.getRooms()]);
