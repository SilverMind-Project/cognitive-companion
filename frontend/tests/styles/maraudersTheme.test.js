...
const vuetifyScss = readFileSync(resolve(process.cwd(), "src/styles/vuetify.scss"), "utf8");
const compiledVuetifyCss = compile(resolve(process.cwd(), "src/styles/vuetify.scss"), {
...
describe("Marauders typography theme", () => {
  it("uses the self-hosted Kalam handwriting family in Marauders mode", () => {
    expect(maraudersCss).toMatch(/\.v-theme--ccMarauders\s*\{[\s\S]*?--cc-font:\s*[\s\S]*?"Kalam"/);
  ...
  it("compiles Vuetify body utilities to --cc-font and heading utilities to --cc-font-display", () => {
    ...
    expect(compiledVuetifyCss).toMatch(/\.text-h4\{[^}]*font-family:var\(--cc-font-display\)/);
    expect(compiledVuetifyCss).toMatch(/\.text-body-2\{[^}]*font-family:var\(--cc-font\)/);
    expect(compiledVuetifyCss).toMatch(/\.text-md-h4\{[^}]*font-family:var\(--cc-font-display\)/);
    expect(compiledVuetifyCss).not.toMatch(/\.text-(?:h4|body-2)\{[^}]*font-family:"Roboto"/);
  });
});
